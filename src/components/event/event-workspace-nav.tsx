  ...
  return (
    <nav aria-label="Event workspace" className="scroll-thin -mx-1 flex gap-1 overflow-x-auto px-1 pb-2">
      {items.map(([suffix, label]) => {
        ...
        return <Link key={key} href={`/events/${eventId}${suffix}`} className={`shrink-0 rounded-full px-4 py-2.5 text-sm font-semibold transition ${selected ? "bg-brand text-brand-ink shadow-sm" : "bg-white text-ink-600 shadow-sm hover:bg-plum-50 hover:text-plum-700"}`}>{label}</Link>;
      })}
