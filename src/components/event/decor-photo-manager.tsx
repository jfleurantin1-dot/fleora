...
export function DecorPhotoManager({eventId,planItemId,itemKey,photos}:{eventId:string;planItemId?:string|null;itemKey:string;photos:Photo[]}){
 ...
 useEffect(()=>{container.current?.dispatchEvent(new CustomEvent("decor-photos-updated",{bubbles:true,detail:{itemKey,photos:localPhotos}}));},[localPhotos,itemKey]);
 ...
 return <div ref={container} className="mt-4 rounded-xl border border-plum-100 bg-plum-50/30 p-3">
   <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold text-ink-800">Inspiration for this item</p><p className="text-[11px] text-ink-500">Upload inspiration pictures, mockups, or examples of what you have in mind.</p></div>{status&&<span className="text-[10px] font-semibold text-plum-700">{status}</span>}</div>
