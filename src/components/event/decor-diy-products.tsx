...
export function DecorCollage({photos,initialActive,initialDiy}:{photos:Photo[];initialActive:string[];initialDiy:string[]}) {
  ...
  useEffect(()=>{
    ...
    const sync=()=>{const selectable=form.querySelectorAll<HTMLInputElement>('input[name^="selected__"]');if(!selectable.length){setActive(initialActive);setDiy(initialDiy);return;}const skip=form.querySelector<HTMLInputElement>('input[name="no_decor"]')?.checked;const keys=skip?[]:Array.from(form.querySelectorAll<HTMLInputElement>('input[name^="selected__"]:checked')).map(i=>i.name.replace("selected__",""));setActive(keys);setDiy(keys.filter(key=>form.querySelector<HTMLInputElement>(`input[name="choice__${key}"]:checked`)?.value==="diy"));};
