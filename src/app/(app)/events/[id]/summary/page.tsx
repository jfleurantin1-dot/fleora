...
function Row({label,status,notes}:{label:string;status:SummaryStatus;notes?:string|null}){
  return <div className="flex flex-wrap items-center justify-between gap-3 border-b fleora-divider py-3 last:border-0">
    <div className="min-w-0"><p className="font-semibold text-ink-900">{label}</p>{status.detail&&<p className="mt-0.5 text-sm text-ink-600">{status.detail}</p>}{notes&&<p className="mt-0.5 max-w-2xl text-xs text-ink-500">{notes}</p>}</div>
...
export default async function PartyPlanSummary({params}:{params:{id:string}}){
  ...
  for(const p of (planItems??[]).filter(p=>p.choice==="undecided")) attention.push({label:p.label,text:"Decision needed",href:p.chapter==="food_drinks"?`/events/${event.id}/plan/food-drinks`:`/events/${event.id}/plan/decor`});
  ...
  return <div className="space-y-7">
    ...
    <Card><div className="flex items-center justify-between"><div><p className="fleora-kicker">Event Details & Vision</p><h2 className="mt-1 font-display text-2xl text-ink-900">{event.name}</h2></div><Badge tone="green"><span className="inline-flex items-center gap-1"><CheckIcon size={12}/> Planned</span></Badge></div><div className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4"><div><p className="text-ink-400">Date</p><b>{event.event_date?shortDate(event.event_date):"TBD"}</b></div><div><p className="text-ink-400">Guests</p><b>{event.guest_count??"TBD"}</b></div><div><p className="text-ink-400">Style</p><b>{event.style||"TBD"}</b></div><div><p className="text-ink-400">Colors</p><b>{event.color_palette||"TBD"}</b></div></div><div className="mt-3"><Row label="Location / Venue" status={venueStatus}/></div></Card>
    {[...groups.entries()].map(([chapter,items])=><Card key={chapter}><div className="flex items-center justify-between gap-3"><div><p className="fleora-kicker">{chapterLabel[chapter]??chapter.replaceAll("_"," ")}</p><h2 className="mt-1 font-display text-2xl text-ink-900">Your decisions</h2></div></div><div className="mt-3">{items.map(item=>{let st=statusFor(item.vendor_category??"",item.choice,activeBookings,quotes??[],requests??[],vendorNames); if(!item.vendor_category&&item.choice==="diy") st={label:"DIY",tone:"green"}; if(item.item_key==="potluck"){const assigned=(potluck??[]).filter(x=>x.assigned_to).length, open=(potluck??[]).length-assigned;st={label:"Planned",tone:"green",detail:`${assigned} assigned${open?` · ${open} still needed`:""}`}; } if(item.item_key==="catering"&&item.choice==="diy"&&(menu??[]).length)st={label:"DIY",tone:"green",detail:(menu??[]).map(m=>m.name).join(" · ")}; return <Row key={item.id} label={item.label} status={st} notes={item.notes}/>})}</div></Card>)}
    {serviceRows.length>0&&<Card><p className="fleora-kicker">Vendors & Services</p><h2 className="mt-1 font-display text-2xl text-ink-900">Your event team</h2><div className="mt-3">{serviceRows.map(c=><Row key={c} label={categoryLabel(c)} status={statusFor(c,undefined,activeBookings,quotes??[],requests??[],vendorNames)}/>)}</div></Card>}
