...
export default async function PartyPlanSummary({params}:{params:{id:string}}){
  ...
  return <div className="space-y-7">
    ...
    <Card><div className="flex items-center justify-between"><div><p className="fleora-kicker">Event Details & Vision</p><h2 className="mt-1 font-display text-2xl text-ink-900">{event.name}</h2></div><Badge tone="green"><span className="inline-flex items-center gap-1"><CheckIcon size={12}/> Planned</span></Badge></div><div className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-5"><div><p className="text-ink-400">Date</p><b>{event.event_date?shortDate(event.event_date):"TBD"}</b></div><div><p className="text-ink-400">Time</p><b>{timeRange((event as any).event_start_time,(event as any).event_end_time)}</b></div><div><p className="text-ink-400">Guests</p><b>{event.guest_count??"TBD"}</b></div><div><p className="text-ink-400">Style</p><b>{event.style||"TBD"}</b></div><div><p className="text-ink-400">Colors</p><b>{event.color_palette||"TBD"}</b></div></div><ul className="mt-3"><Row label="Location / Venue" status={venueStatus}/></ul></Card>
