...
function chapterHref(eventId:string,chapter:string){
  if(chapter==="food_drinks") return `/events/${eventId}/plan/food-drinks`;
  if(chapter==="services") return `/events/${eventId}/services`;
  if(chapter==="entertainment") return `/events/${eventId}/entertainment`;
  if(chapter==="venue_logistics") return `/events/${eventId}/venue-logistics`;
  return `/events/${eventId}/plan/decor`;
}
...
function Row({label,status,notes,href}:{label:string;status:SummaryStatus;notes?:string|null;href?:string}){
  const content=<>
...
export default async function PartyPlanSummary({params}:{params:{id:string}}){
  ...
  return <div className="space-y-7">
    ...
    <Card><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="fleora-kicker">Event Details & Vision</p><h2 className="mt-1 font-display text-2xl text-ink-900">{event.name}</h2></div><div className="flex items-center gap-2"><Badge tone="green"><span className="inline-flex items-center gap-1"><CheckIcon size={12}/> Planned</span></Badge><ButtonLink href={`/events/${event.id}/edit`} variant="secondary" size="sm">Edit details</ButtonLink></div></div><div className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-5"><div><p className="text-ink-400">Date</p><b>{event.event_date?shortDate(event.event_date):"TBD"}</b></div><div><p className="text-ink-400">Time</p><b>{timeRange((event as any).event_start_time,(event as any).event_end_time)}</b></div><div><p className="text-ink-400">Guests</p><b>{event.guest_count??"TBD"}</b></div><div><p className="text-ink-400">Style</p><b>{event.style||"TBD"}</b></div><div><p className="text-ink-400">Colors</p><b>{event.color_palette||"TBD"}</b></div></div><ul className="mt-3"><Row label="Location / Venue" status={venueStatus} href={`/events/${event.id}/edit`}/></ul></Card>
    {[...groups.entries()].map(([chapter,items])=>{const href=chapterHref(event.id,chapter);return <Card key={chapter}><div className="flex items-center justify-between gap-3"><h2 className="font-display text-2xl text-ink-900">{chapterLabel[chapter]??chapter.replaceAll("_"," ")}</h2><ButtonLink href={href} variant="secondary" size="sm">Edit chapter</ButtonLink></div><ul className="mt-3 space-y-0.5">{items.map(item=>{let st=statusFor(item.vendor_category??"",item.choice,activeBookings,quotes??[],requests??[],vendorNames); if((item.chapter==="services"||item.chapter==="entertainment")&&(item.choice==="diy"||item.choice==="existing")) st={label:"Planned",tone:"green",detail:item.item_key==="no_entertainment"||item.item_key==="no_services"?"None needed":"Already have someone"}; if(!item.vendor_category&&item.choice==="diy") st={label:"DIY",tone:"green"}; if(["no_decor","skip_food_drinks","no_services","no_entertainment","no_venue_logistics"].includes(item.item_key)) st={label:"Complete",tone:"green",detail:item.item_key==="skip_food_drinks"?"Skipped in Fleora":"None needed"}; if(item.item_key==="potluck"){const assigned=(potluck??[]).filter(x=>x.assigned_to).length, open=(potluck??[]).length-assigned;st={label:"Planned",tone:"green",detail:`${assigned} assigned${open?` · ${open} still needed`:""}`}; } if(item.item_key==="catering"&&item.choice==="diy"&&(menu??[]).length)st={label:"DIY",tone:"green",detail:(menu??[]).map(m=>m.name).join(" · ")}; return <Row key={item.id} label={item.label} status={st} notes={item.notes} href={href}/>})}</ul></Card>})}
    {serviceRows.length>0&&<Card><div className="flex items-center justify-between gap-3"><div><p className="fleora-kicker">Vendors & Services</p><h2 className="mt-1 font-display text-2xl text-ink-900">Your event team</h2></div><ButtonLink href={`/events/${event.id}/vendors`} variant="secondary" size="sm">Manage vendors</ButtonLink></div><ul className="mt-3 space-y-0.5">{serviceRows.map(c=><Row key={c} label={categoryLabel(c)} status={statusFor(c,undefined,activeBookings,quotes??[],requests??[],vendorNames)} href={`/events/${event.id}/vendors`}/>)}</ul></Card>}
    <Card><div className="flex items-center justify-between gap-3"><div><p className="fleora-kicker">Guests</p><h2 className="mt-1 font-display text-2xl text-ink-900">RSVP snapshot</h2></div><ButtonLink href={`/events/${event.id}/guests`} variant="secondary" size="sm">Manage guests</ButtonLink></div><p className="mt-3 text-sm text-ink-600"><b className="text-ink-900">{invited}</b> invited · <b className="text-ink-900">{attending}</b> attending · <b className="text-ink-900">{pending}</b> pending</p></Card>
    <Card variant="feature" className="bg-brand-soft"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="fleora-kicker">Ready to source it?</p><h2 className="mt-1 font-display text-2xl">Turn your Hire choices into your vendor search.</h2><p className="mt-1 text-sm text-ink-600">See every vendor your party needs in one place and start with Fleora matches.</p></div><ButtonLink href={`/events/${event.id}/vendors`}>Find My Vendors →</ButtonLink></div></Card>
