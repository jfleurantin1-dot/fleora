...
export default async function PartyPlanSummary({params}:{params:{id:string}}){
  ...
  for(const p of (planItems??[]).filter(p=>p.choice==="undecided")) attention.push({label:p.label,text:"Decision needed",href:p.chapter==="food_drinks"?`/events/${event.id}/plan/food-drinks`:p.chapter==="services"?`/events/${event.id}/services`:`/events/${event.id}/plan/decor`});
