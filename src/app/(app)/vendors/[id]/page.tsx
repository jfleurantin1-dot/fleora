    ...
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mb-6">
        <div className="grid gap-2 overflow-hidden rounded-[24px] bg-plum-50 sm:grid-cols-3">
          {photos && photos.length > 0 ? photos.slice(0,3).map((p,index) => <div key={p.id} className="relative min-h-72 sm:min-h-[360px]"><Image src={p.url} alt={index===0?vendor.business_name:`${vendor.business_name} portfolio photo ${index+1}`} fill sizes="(max-width:640px) 100vw,33vw" className="object-cover" /></div>) : <div className="col-span-full grid min-h-72 place-items-center bg-brand-soft"><span className="font-display text-7xl text-plum-300">F</span></div>}
        </div>
