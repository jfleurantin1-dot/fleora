...
export default async function BrowseVendors(
 ...
 const vendors=(allVendors??[]).filter(v=>{if(profile.account_type==="vendor"&&v.user_id===profile.id)return false;const vc=byCat.get(v.id)??[];const price=minPrice.get(v.id);if(q&&!`${v.business_name} ${v.description??""} ${v.location??""} ${vc.map(categoryLabel).join(" ")}`.toLowerCase().includes(q))return false;if(category&&!vc.includes(category))return false;if(location&&!(v.location??"").toLowerCase().includes(location))return false;if(maxPrice!=null&&price!=null&&price>maxPrice)return false;if(date&&blockedSet.has(v.id))return false;if(savedOnly&&!favoriteSet.has(v.id))return false;return true});
 return <div><PageHeader title={profile.account_type==="vendor"?"Discover Vendors":"Find vendors"} subtitle={profile.account_type==="vendor"?"Meet other Fleora vendors for referrals, collaborations and networking.":"Search Fleora by service, location, price and availability."}/>
