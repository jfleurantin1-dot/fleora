...
export default async function VendorDashboard(){
 ...
 return <div>
  ...
  {newLeads.length>0&&<Card variant="feature" className="mb-8 flex flex-wrap items-center justify-between gap-4"><div><p className="fleora-kicker">Action needed</p><p className="mt-1 font-display text-2xl text-ink-900">You have {newLeads.length} inquir{newLeads.length===1?"y":"ies"} waiting</p><p className="mt-1 text-sm text-ink-600">Review the event details, message the client, and send a quote while the opportunity is fresh.</p></div><ButtonLink href="/vendor/leads" size="sm">Review inquiries →</ButtonLink></Card>}
  <div className="mb-8 grid gap-6 lg:grid-cols-[1.15fr_.85fr]">
   <Card padding="lg"><div className="flex items-start justify-between gap-4"><div><p className="fleora-kicker">Profile strength</p><h2 className="mt-1 font-display text-2xl text-ink-900">Make your storefront booking-ready</h2></div><span className="text-xl font-semibold text-plum-700">{completion}%</span></div><div className="mt-4"><Progress value={completion}/></div><div className="mt-5 grid gap-2 sm:grid-cols-2">{checks.map(c=><div key={c.label} className={`rounded-2xl p-3 ${c.done?"bg-emerald-50":"bg-ivory-100"}`}><div className="flex gap-2"><span className={c.done?"text-emerald-600":"text-ink-400"}>{c.done?"✓":"○"}</span><div><p className="text-sm font-semibold text-ink-900">{c.label}</p><p className="mt-0.5 text-xs leading-relaxed text-ink-500">{c.detail}</p></div></div></div>)}</div><div className="mt-4"><ButtonLink href="/vendor/onboarding" variant="secondary" size="sm">{completion>=100?"Edit my profile":"Complete my profile"}</ButtonLink></div></Card>
   <Card variant="soft" padding="lg"><p className="fleora-kicker">Quick actions</p><h2 className="mt-1 font-display text-2xl text-ink-900">Run your business</h2><div className="mt-5 grid gap-2"><ButtonLink href="/vendor/leads" variant="secondary" className="justify-between">Review inquiries <span>→</span></ButtonLink><ButtonLink href="/messages" variant="secondary" className="justify-between">Open messages <span>→</span></ButtonLink><ButtonLink href="/vendor/onboarding" variant="secondary" className="justify-between">Update services & portfolio <span>→</span></ButtonLink><ButtonLink href="/vendor/availability" variant="secondary" className="justify-between">Manage availability <span>→</span></ButtonLink><ButtonLink href="/vendor/payments" variant="secondary" className="justify-between">Payments & payouts <span>→</span></ButtonLink><ButtonLink href="/vendor/claim" variant="ghost" className="justify-between">Find & claim another profile <span>→</span></ButtonLink></div>{packages?.length? <p className="mt-4 text-xs text-ink-500">{packages.length} package{packages.length===1?"":"s"} currently on your profile.</p>:null}</Card>
  </div>
  <div className="grid gap-8 lg:grid-cols-2"><section><SectionHeader title="New inquiries" description="Fresh requests that still need your response." action={<Link href="/vendor/leads" className="text-sm font-semibold text-plum-700">View all →</Link>}/>{!newLeads.length?<Empty title="You’re caught up">New client inquiries will appear here.</Empty>:<ul className="space-y-3">{newLeads.slice(0,4).map(c=>{const e=eventMap.get(c.event_id); return <Card as="li" key={c.id} variant="interactive" padding="none"><Link href={`/vendor/inquiries/${c.id}`} className="block p-5"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold text-ink-900">{e?.name??"New event inquiry"}</p><p className="mt-1 text-sm text-ink-600">{shortDate(e?.event_date)} · {e?.location??"TBD"} · {e?.guest_count??"?"} guests</p></div><Badge tone="amber">Needs quote</Badge></div><div className="mt-3 flex flex-wrap gap-2">{e?.style&&<Badge tone="plum">{e.style}</Badge>}<Badge tone="champagne">{money(e?.budget)} budget</Badge></div></Link></Card>})}</ul>}</section>
