...
export default async function VendorDashboard(){
 ...
 return <div>
  ...
  <div className="mb-8 grid gap-6 lg:grid-cols-[1.15fr_.85fr]">
   ...
   <Card variant="soft" padding="lg"><p className="fleora-kicker">Quick actions</p><h2 className="mt-1 font-display text-2xl text-ink-900">Run your business</h2><div className="mt-5 grid gap-2"><ButtonLink href="/vendor/leads" variant="secondary" className="justify-between">Review leads <span>→</span></ButtonLink><ButtonLink href="/messages" variant="secondary" className="justify-between">Open messages <span>→</span></ButtonLink><ButtonLink href="/vendor/onboarding" variant="secondary" className="justify-between">Update services & portfolio <span>→</span></ButtonLink><ButtonLink href="/vendor/claim" variant="ghost" className="justify-between">Find & claim another profile <span>→</span></ButtonLink></div>{packages?.length? <p className="mt-4 text-xs text-ink-500">{packages.length} package{packages.length===1?"":"s"} currently on your profile.</p>:null}</Card>
  </div>
