...
export default async function QuotePage({params}:{params:{id:string}}){
 ...
 return <div className="mx-auto max-w-2xl">
  ...
  <Card variant="feature" padding="lg" className="overflow-hidden">
   ...
   {canDecide?<div className="mt-6 space-y-3"><form action={accept}><Button type="submit" size="lg" className="w-full">Accept quote</Button></form><details className="rounded-2xl border border-rose-100 bg-white p-4"><summary className="cursor-pointer text-center text-sm font-semibold text-rose-700">Decline quote</summary><form action={decline} className="mt-4 space-y-3"><select name="reason" required className="w-full rounded-xl border border-plum-100 bg-white px-3 py-2.5 text-sm"><option value="">Choose a reason</option><option value="budget">Price is outside my budget</option><option value="another_vendor">I chose another vendor</option><option value="plans_changed">My plans changed</option><option value="no_longer_needed">I no longer need this service</option><option value="not_a_fit">The quote is not the right fit</option><option value="other">Other</option></select><textarea name="note" rows={3} placeholder="Optional note (kept private unless you message the vendor)" className="w-full rounded-xl border border-plum-100 px-3 py-2.5 text-sm"/><Button type="submit" variant="secondary" size="lg" className="w-full">Decline quote</Button></form></details></div>:quote.status==="accepted"&&isClient?<div className="mt-6 rounded-2xl border border-[#E7DFEA] bg-ivory-50 p-5"><div className="flex items-start justify-between gap-4"><div><p className="fleora-kicker">Fleora Pay</p><h2 className="mt-1 font-serif text-2xl text-ink-900">{remaining>0?"Secure your booking":"Paid in full"}</h2><p className="mt-1 text-sm text-ink-600">{remaining>0?`Paid ${money(paid)} · ${money(remaining)} remaining`:"Your booking payment is complete."}</p></div>{booking&&<Badge tone={booking.status==="confirmed"?"green":"amber"}>{booking.status.replace("_"," ")}</Badge>}</div>{remaining>0?vendorReady?<div className="mt-5 grid gap-3 sm:grid-cols-2">{paid===0&&depositDue>0&&depositDue<Number(quote.total)&&<form action={startQuotePayment.bind(null,quote.id,"deposit")}><Button type="submit" size="lg" className="w-full">Pay {money(depositDue)} deposit</Button></form>}<form action={startQuotePayment.bind(null,quote.id,paid>0?"balance":"full")}><Button type="submit" variant={paid===0&&depositDue>0&&depositDue<Number(quote.total)?"secondary":"primary"} size="lg" className="w-full">{paid>0?`Pay ${money(remaining)} balance`:`Pay ${money(remaining)} in full`}</Button></form></div>:<p className="mt-4 rounded-xl bg-blush-50 px-4 py-3 text-sm text-ink-700">This vendor is finishing Stripe payout setup. Payment will unlock as soon as their account is ready.</p>:<p className="mt-4 rounded-xl bg-sage-50 px-4 py-3 text-sm text-ink-700">✓ Payment complete. Your booking is confirmed.</p>}</div>:<p className="mt-6 rounded-xl bg-ivory-100 px-4 py-3 text-sm text-ink-600">{quote.status==="declined"?"This quote was declined.":"This quote is no longer actionable."}</p>}
  </Card>
