...
export default async function PaymentDetail({params}:{params:{id:string}}){
 ...
 return <div className="mx-auto max-w-2xl"><PageHeader title="Payment details" subtitle={`${event?.name??"Event"} · ${vendor?.business_name??"Vendor"}`}/>
  ...
  <div className="mt-6 flex flex-wrap gap-3">{p.receipt_url&&<a href={p.receipt_url} target="_blank" rel="noreferrer" className="inline-flex min-h-12 items-center justify-center rounded-[10px] bg-brand px-4 py-2.5 text-sm font-medium text-brand-ink shadow-sm">Open Stripe receipt</a>}<ButtonLink href={`/events/${p.event_id}/payments`} variant="secondary">Event payments</ButtonLink>{booking?.quote_id&&<ButtonLink href={`/quotes/${booking.quote_id}`} variant="ghost">View quote</ButtonLink>}</div>
