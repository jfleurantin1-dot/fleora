    ...
    <header className="sticky top-0 z-50 border-b border-[#EDE7F1] bg-white/95 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-8">
        <div className="flex min-w-0 items-center gap-8">
          <BrandLogo href={isVendor?"/vendor/dashboard":"/dashboard"} compact className="shrink-0"/>
          <DesktopAppNav isVendor={isVendor} isAdmin={isAdmin}/>
        </div>
        <div className="flex shrink-0 items-center gap-1 sm:gap-2">
          <NotificationBell userId={profile.id}/>
          <ProfileMenu name={name} initials={initials(profile.first_name,profile.last_name)} isVendor={isVendor}/>
        </div>
      </div>
    </header>
    <main className="mx-auto max-w-7xl px-5 py-6 sm:px-8 sm:py-8">{children}</main>
    <MobileAppNav isVendor={isVendor} isAdmin={isAdmin}/>
  </div>;
}
